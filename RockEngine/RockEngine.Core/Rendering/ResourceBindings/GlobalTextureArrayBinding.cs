...
namespace RockEngine.Core.Rendering.ResourceBindings
{
    public sealed class GlobalTextureArrayBinding : ResourceBinding
    {
        ...
        public unsafe override void UpdateDescriptorSet(VulkanContext context, uint frameIndex, VkDescriptorSetLayout layout)
        {
            ...
            try
            {
                ...
                fixed (DescriptorImageInfo* pImageInfos = imageInfos)
                {
                    ...
                    VK.UpdateDescriptorSets(context.Device, (uint)validCount, pWrites, 0, null);
                }
            }
            finally
            {
                ArrayPool<WriteDescriptorSet>.Shared.Return(writes, false);
                ArrayPool<DescriptorImageInfo>.Shared.Return(imageInfos, false);
            }
        }
