    ...
    internal class DeferredRenderPipeline : IRenderPipeline
    {
        ...
        private async Task ExecuteCameraPass(VkCommandBuffer cmd, Camera camera, Renderer renderer, int camIndex)
        {
            ...
            for (int i = 0; i < buffers.Length; i++)
            {
                ...
                VkCommandBuffer vkCommandBuffer = buffers[i];
                vkCommandBuffer.Begin(CommandBufferUsageFlags.RenderPassContinueBit, in inheritanceInfo);

                vkCommandBuffer.LabelObject($"ExecuteCameraPass [{i}] cmd");
            }
            using (cmd.NameAction("Geometry Pass", [0.8f, 0.2f, 0.2f, 1.0f]))
            {
                await _geometryPass.Execute(buffers[0], renderer.FrameIndex, camera, camIndex);
                buffers[0].End();
            ...
            {
                cmd.NextSubpass(SubpassContents.SecondaryCommandBuffers);
                await _postLightPass.Execute(buffers[2], renderer.FrameIndex, camera, camIndex);
                buffers[2].End();
