...
namespace RockEngine.Core
{
    ...
    {
        /// <summary>Called once after window and graphics are ready.</summary>
        Task InitializeAsync(GraphicsContext graphics, WorldRenderer renderer, World world);

        /// <summary>Called every frame before rendering.</summary>
        Task UpdateAsync();

        /// <summary>Called every frame to issue rendering commands.</summary>
        Task RenderAsync(RenderContext renderContext);

        /// <summary>Called when the application is shutting down.</summary>
        Task ShutdownAsync();
    }
